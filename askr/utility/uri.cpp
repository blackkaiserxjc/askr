    ...
    : has_authority_(false), port_(0)
{
    ...
    std::cmatch match;
    if (!std::regex_match(str.begin(), str.end(), match, uri_regex))
    {
    ...
    if (!std::regex_match(auth_path.begin(), auth_path.end(), auth_path_match,
            auth_path_regex))
    {
    ...
    else
    {
        ...
        if (!std::regex_match(auth.first, auth.second, auth_match, auth_regex))
        {
            throw std::invalid_argument(
                fmt::format("invalid uri authority {} {}", auth.first, auth.second));
        }
        ...
        if (!port.empty())
        {
            ...
            {
                throw std::invalid_argument(
                    fmt::format("invalid uri port: ", e.what()));
            }
        }
