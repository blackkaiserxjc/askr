...
struct span_is_span<askr::span<T, E>>
{
...
{
    static constexpr bool value = (N == dynamic_extent || span_capacity<E, N>::value) && span_convertible<U, T>::value;
};

template <std::size_t E, std::size_t O>
struct span_sub
{
    static constexpr std::size_t value = E == dynamic_extent ? dynamic_extent : E - O;
};
...
template <class T, std::size_t E>
inline span<const std::uint8_t, detail::span_bytes<T, E>::value>
as_bytes(span<T, E> s) noexcept
{
    return span<const std::byte, detail::span_bytes<T, E>::value>(reinterpret_cast<const std::uint8_t *>(s.data()),
        s.size_bytes());
}

template <class T, std::size_t E>
inline typename std::enable_if<!std::is_const<T>::value,
    span<std::uint8_t, detail::span_bytes<T, E>::value>>::type
as_writable_bytes(span<T, E> s) noexcept
{
    return span<std::byte, detail::span_bytes<T, E>::value>(reinterpret_cast<std::uint8_t *>(s.data()), s.size_bytes());
}
